...
function smartPublishComposite(name, pub) {
    if (Meteor.isServer) {
        if (_.isObject(pub)) {
            ...
            throw new Meteor.Error('argument pub must be an object');
...
function mapToCursor(pub, ...subArgs) {
    var self = this;
...
function createCompositeMethod(pub, parentName, meth) {
